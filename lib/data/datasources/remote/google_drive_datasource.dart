...
class GoogleDriveDataSource {
  ...
  Future<_DriveClientContext> _driveApi() async {
  ...
  Future<void> deleteBackup(String fileId) async {
    await _withDriveApi((api) async => api.files.delete(fileId));
  }
  ...
  Future<String?> downloadBackupChecksumByName(String checksumFilename) async {
    return _withDriveApi((api) async {
      ...
      final text = await _downloadFileAsStringById(api, fileId);
      final normalized = text.trim();
      return normalized.isEmpty ? null : normalized;
    });
  }

  Future<String> _downloadFileAsStringById(drive.DriveApi api, String fileId) async {
    final response = await api.files.get(
      fileId,
      downloadOptions: drive.DownloadOptions.fullMedia,
    ) as drive.Media;
...
class _DriveClientContext {
  _DriveClientContext(this.api, this.client);
